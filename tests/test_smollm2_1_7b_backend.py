...
from models.smollm2_1_7b import MODEL
...
def _make_backend(completions):
    ...
    backend = SmolLM2LargeAuditBackend(
...
class TestBackend:
    def test_shares_the_parametric_implementation(self):
        assert issubclass(SmolLM2LargeAuditBackend, SmolLM2AuditBackend)

    def test_three_states_identical_and_valid(self):
        backend, model = _make_backend({"France capital is": "Paris . more text"})
    ...
    def test_resume_identity_names_this_model_and_class(self):
        ...
        assert identity["class"].endswith("SmolLM2LargeAuditBackend")
        # Distinct from the 360M backend's identity even at equal prompts.
        assert "smollm2_1_7b" in identity["class"]


class TestRegistration:
    def test_backend_registered(self):
        assert "smollm2-1.7b" in available_backends()

    def test_no_oracle_bootstrap_capability(self):
        assert not get_backend_spec("smollm2-1.7b").supports_oracle_bootstrap

    def test_model_is_the_nulls_scale_reference(self):
        assert MODEL == "HuggingFaceTB/SmolLM2-1.7B"
...
class TestValidate:
    def test_accepts_plain_standard_audit(self):
        get_backend_spec("smollm2-1.7b").validate(_args())

    def test_requires_prompt_files(self):
        with pytest.raises(ValueError, match="--prompt-files"):
            get_backend_spec("smollm2-1.7b").validate(_args(prompt_files=None))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"bootstrap_oracle_from_full": True}, "--bootstrap-oracle-from-full"),
        ],
    )
    def test_rejects_retrieval_modes(self, overrides, flag):
        with pytest.raises(ValueError, match=re.escape(flag)):
            get_backend_spec("smollm2-1.7b").validate(_args(**overrides))
